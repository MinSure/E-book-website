...
def index(request):
    ...
    # for
    #每种类型按new分类返回
    python_new = Book.objects.get_books_by_type(1, limit=3, sort='new')
    #python_hot = Books.objects.get_books_by_type(PYTHON, limit=4, sort='view-hot')
    javascript_new = Book.objects.get_books_by_type(2,limit= 3, sort='new')
    #javascript_hot = Books.objects.get_books_by_type(JAVASCRIPT, limit=4, sort='view-hot')
    algorithms_new = Book.objects.get_books_by_type(3, 3, sort='new')
